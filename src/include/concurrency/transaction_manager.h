...
class TransactionManager {
 public:
  ...
  /**
   * Begins a new transaction. 开始一个事务
   * @param txn an optional transaction object to be initialized, otherwise a new transaction is created.
   * @param isolation_level an optional isolation level of the transaction. 默认隔离级别是可重复读
  ...
  /**
   * Commits a transaction. 事务提交
   * @param txn the transaction to commit
   */
  void Commit(Transaction *txn);

  /**
   * Aborts a transaction 事务终止
  ...
  /** Resumes(回收) all transactions, used for checkpointing. */
