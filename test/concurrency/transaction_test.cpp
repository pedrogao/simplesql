...
TEST_F(TransactionTest, SimpleUpdateRollbackTest) {
  ...
  // txn2: SELECT * FROM empty_table2;
  // txn3: UPDATE empty_table2 SET colA = 0 WHERE
  // txn3: abort
  ...
  result_set.clear();
  GetExecutionEngine()->Execute(&update_plan, &result_set, txn2, exec_ctx2.get());

  ASSERT_EQ(result_set.size(), 3);
  Tuple tuple0 = result_set[0];
  const Value &value = tuple0.GetValue(&schema, 0);
  ASSERT_EQ(value.GetAs<int>(), 1);
  ...
  tuple0 = result_set[0];
  const Value &value1 = tuple0.GetValue(&schema, 0);
  ASSERT_EQ(value1.GetAs<int>(), 200);
